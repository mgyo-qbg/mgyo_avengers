...
**플러그인 파일 위치**: `/Users/imsi/Desktop/betterwealth-figma-plugin/`
...
3. `/Users/imsi/Desktop/betterwealth-figma-plugin/manifest.json` 선택
4. 실행
...
## 오늘 작업 이력 요약 (#1114 인출설계 설문지)

### 확정된 정책
- 설문 완료(FA 전달) 시 진입점 자체 미노출 (재진단 불가)
- 이어가기: 임시저장 만료 없음 (회원 탈퇴 전까지 영구 보관)
- 재진입 시 마데/DC 판단 재호출 (마이데이터 신규 연동 가능성)
- Yes/No 선택: 피커(세그먼트 컨트롤) 사용
- 설문 입력 페이지(P2~P11): 스텝 인디케이터 표시 (N/10)

### 최종 플러그인 (14개 페이지)
| 페이지 | 내용 |
|--------|------|
| P1 | 설문 시작 |
| P2~P11 | 설문 입력 (스텝인디케이터 1~10/10) |
| P12 | 이어하기 선택 (임시저장 있을 때) |
| P13 | 전송 로딩 |
| P14 | 전달 완료 |

### 미확인 사항
- 마이데이터 절세형/일반형 자산 기준 — 앱 계좌 타입 별도 확인 필요
- DC 자산 보유여부 기준 — 앱 계좌 타입 별도 확인 필요
