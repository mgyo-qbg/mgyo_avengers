    ...
    └─ Y → 설문입력 시작 페이지 (P1)
           ...
           [?] 마데 절세형/일반형 자산 보유 Y/N  ← 백엔드 판단
               ...
               └─ Y → 은퇴시기 입력 페이지 (P4)
                       ...
                       은퇴후 생활비 입력 페이지 (P6)
                           ↓
                       국민연금 수령 여부 + 월수령액 입력 [병합] (P7)
                           │  "수령 중" → 월수령액 직접입력 (CTA 즉시 활성)
                           │  "수령 전" → 월수령액 입력 + 계산하기 링크 (바텀시트 계산기 — 연소득/최초가입시기/납입종료) → 계산 완료 후 CTA 활성
                           ↓
...
flowchart TD
    ...
    subgraph P7["P7 국민연금 입력 [병합]"]
        NPS_YN{국민연금을 수령 중이신가요?}
        NPS_YN -- 수령 중 --> NPS_YES[수령 중 — 월수령액 입력]
        NPS_YN -- 수령 전 --> NPS_NO[수령 전 — 월수령액 입력 + 계산하기 바텀시트]
    end
...
| P7. 국민연금 수령 여부 + 입력 [병합] | 7262:1668 | 1664397177 | "수령 중"→월수령액 직접입력(세전/만원) CTA 즉시 활성 / "수령 전"→월수령액 입력 + 계산하기 링크(바텀시트) — 계산기(연소득+최초가입시기+납입종료readonly) / 납입종료: **만 60세 해당달 자동 입력** (readonly) / 계산하기 완료 후 "다음" CTA 활성화 |
...
| 2026-03-12 | P7 UX 전면 개편 — 질문 변경("국민연금을 수령 중이신가요?"), 수령 중/전 선택 무관 월수령액 입력폼 항상 표시, 계산기 바텀시트로 분리 |
| 2026-03-12 | 납입 종료 예정일 — 은퇴시기 연동(readonly) → 만 60세 해당달(birthYear+59, birthMonth 유지)로 변경 (demo.html + code.js 동기화) |
| 2026-03-12 | FC_NODES P7 노드 라벨 동기화 — NPS_YN "수령 중이신가요?", NPS_YES "수령 중 — 월수령액 입력", NPS_NO "수령 전 — 월수령액 입력 + 계산하기 바텀시트" |
