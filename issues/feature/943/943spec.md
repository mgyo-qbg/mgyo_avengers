...
- 2026-03-11: 플로우 수정
  ...
  - 프로그레스 바: 10칸 세그먼트 → 연속형 바 (N/8 텍스트 + DS Progress Bar 컴포넌트 표준)
    ...
    └─ Y → 설문입력 시작 페이지 (P1)
           ...
           [?] 마데 절세형/일반형 자산 보유 Y/N  ← 백엔드 판단
               ...
               └─ Y → 은퇴시기 입력 페이지 (P4)
                       ...
                       은퇴후 생활비 입력 페이지 (P6)
                           ↓
                       국민연금 예상 수령액을 아시나요? (P7)
                           ↓
                       [?] 안다 / 모른다 Y/N
                           ├─ 모른다 → 국민연금 계산기 입력 페이지 (P8)
                           └─ 안다   → 국민연금 월수령액 직접입력 페이지 (P9)
                                           ↓ (합류)
                                       [?] DC 자산 보유여부 Y/N  ← 백엔드 판단
                                           ├─ Y → 마이데이터 IRP/DC 자산 확인 페이지 (P10)
                                           │           ↓
                                           │       기타소득 질문+입력 [병합] (P12)  ← 직행
                                           └─ N → 근로소득자 Y/N + 연봉/근속연수 [병합] (P11)
                                                       │  "네" 선택 시 연봉/근속연수 입력 영역 펼침
                                                       │  "아니요" 선택 시 입력 없이 진행
                                                       ↓
                                                   기타소득 질문+입력 [병합] (P12)
                                                       │  "네" 선택 시 연소득/시작시기/종료시기 영역 펼침
                                                       │  "아니요" 선택 시 입력 없이 진행
                                                       ↓
                                                   설문 결과전달 페이지 (P13)
                                                       ↓
...
flowchart TD
  ...
  MYDATA_YN -- Y --> EXPENSE[P4. 은퇴시기 입력]

  EXPENSE --> EXPENSE2[P5. 기대수명 입력]
  EXPENSE2 --> EXPENSE3[P6. 은퇴후 생활비 입력]

  EXPENSE3 --> NPS_Q[P7. 국민연금 예상 수령액을 아시나요?]
  NPS_Q --> NPS_YN{안다 / 모른다?}
  NPS_YN -- N --> NPS_CALC[P8. 국민연금 계산기]
  NPS_YN -- Y --> NPS_DIRECT[P9. 국민연금 월수령액 직접입력]

  NPS_CALC --> DC_YN
  NPS_DIRECT --> DC_YN{DC 자산 보유여부?}

  DC_YN -- Y --> IRP_DC[P10. 마이데이터 IRP/DC 자산 확인]
  IRP_DC --> EXTRA_Q
  DC_YN -- N --> WORKER_Q["P11. 근로소득자 Y/N + 연봉·근속연수 입력 [병합]<br/>네→연봉/근속연수 펼침 / 아니요→바로 진행"]
  WORKER_Q --> EXTRA_Q

  EXTRA_Q["P12. 기타소득 Y/N + 연소득 입력 [병합]<br/>네→소득/시작/종료 펼침 / 아니요→바로 진행"] --> RESULT[P13. 설문 결과전달 페이지]
...
| P8. 국민연금 계산기 | 7262:1668 | 1664397177 | 연소득: 세전/만원/디폴트없음, 최초가입시기: 연+월, 예상납입종료: 만 60세 자동입력(readonly) |
| P9. 국민연금 직접입력 | 7262:1669 | 1664397392 | 세전 월수령액, 단위 만원 |
| P10. DC 자산 보유 판단 | 7262:1654 | 1664397562 | 백엔드 판단: (INV, PNS, DC_) 자산 존재 여부, ⚠️ 앱 계좌 타입 별도 확인 필요 |
| P11. 근로소득자 Y/N + 연봉·근속연수 [병합] | 7262:1677 | 1664397956 | Y 선택 시 연봉/근속연수 입력 영역 펼침. 연봉: 세전/만원, P8 계산기 경로면 P8 연소득 디폴트(수정가능) / 직접입력 경로면 디폴트없음 / 근속연수: 현재 재직 회사 기준 만(滿) 연수 |
| P12. 기타소득 Y/N + 입력 [병합] | 7262:1641 | 1664398710 | Y 선택 시 소득 입력 영역 펼침. 연소득(세전) 단일 입력, 소득유형 미표시, 시작시기=은퇴시기(readonly), 종료시기=기대수명(readonly) |
...
> ⚠️ 병합 후 번호: 로딩=P15, 완료=P16 (구 P17, P18)
